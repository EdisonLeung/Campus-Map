...
public final class RatTerm {
    ...
    public double eval(double d) {
        return coeff.doubleValue() * Math.pow(d, this.expt);
    }
    ...
    public RatTerm add(RatTerm arg) {
        if (this.expt != arg.expt &&
                !this.isZero() && !arg.isZero() && !this.isNaN() && !arg.isNaN()) {
            throw new IllegalArgumentException();
        }

        if (this.isNaN() || arg.isNaN()) { return NaN; }
    ...
    public RatTerm div(RatTerm arg) {
        if (this.isNaN() || arg.isNaN() || arg.isZero()){ return NaN; }
